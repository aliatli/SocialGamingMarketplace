...
if(isset($_SESSION['UserID'])){
  ...
  $sql_follow = "SELECT * FROM BlockList WHERE UserUserID = $user_id AND UserUserID2 = $profile_id;";
  $result_follow = mysqli_query($conn, $sql_follow);
  $result_follow_check = mysqli_num_rows($result_follow);

  if($result_follow_check === 0){
    $sql_insert = "INSERT INTO BlockList VALUES ($user_id, $profile_id);";
    mysqli_query($conn, $sql_insert);
  }
  else{
    $sql_delete = "DELETE FROM BlockList WHERE UserUserID = $user_id AND UserUserID2 = $profile_id;";
